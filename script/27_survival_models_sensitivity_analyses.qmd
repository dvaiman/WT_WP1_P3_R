...
sens_m3 <- bind_rows(
  # exclude anyone with CVD before their first HPA (referee 2.7)
  fit_m3_sens(
    data_cvd |> filter(cvd_art_before_HPA_flag == 0),
    "M3, excluding prior CVD"
  ),
  # restrict to working ages 18–65 at first HPA (referee 2.13).
  # NB: this drops some matched cluster members; we refit directly on the
  # restricted set rather than re-matching.
  fit_m3_sens(
    data_cvd |> filter(Age >= 18 & Age <= 65),
