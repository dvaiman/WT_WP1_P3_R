...
#     • at, t     : exactly as in standsurv() function
#     • type      : "hazard", "rmst", "survival", …
#     • contrast  : "ratio", "difference", or NULL
#     • ...       : other arguments to pass to standsurv()
# ──────────────────────────────────────────────────────────────────────

quick_standsurv <- function(
  fit,
  at,
  t,
  type = c("hazard", "rmst", "survival", "cumhaz", "quantile"),
...
fast_stand <- function(
  ...
  at,
  t,
  type = c("rmst", "hazard", "survival", "cumhaz", "quantile"),
  ...
  uniq <- data |>
    dplyr::count(
      dplyr::across(dplyr::all_of(collapse_vars)),
      name = "(weights)"
    ) |>
  ...
  message(
    ...
    type,
    ", contrast = ",
    ifelse(is.null(contrast), "none", contrast),
    if (ci) {
      paste0(
        "  (CI via ",
        ci_method,
        if (ci_method == "boot") paste0(", B = ", B),
        ")"
      )
    } else {
      ""
