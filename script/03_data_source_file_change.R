...
df_change <- data_cvd |>
  mutate(
    ...
    Exercise_change_first_to_second = case_when(
      ...
      Exercise_cat == "Low" & Exercise_cat_second %in% c("Moderate", "Good") ~
        "Improvement",
      Exercise_cat == "Moderate" & Exercise_cat_second == "Good" ~
        "Improvement",
      Exercise_cat == "Good" & Exercise_cat_second %in% c("Low", "Moderate") ~
        "Worsening",
      Exercise_cat == "Moderate" & Exercise_cat_second == "Low" ~ "Worsening",
      TRUE ~ "No change"
    ),
    ...
    Exercise_change_first_to_last = case_when(
      ...
      Exercise_cat == "Low" & Exercise_cat_last %in% c("Moderate", "Good") ~
        "Improvement",
      Exercise_cat == "Moderate" & Exercise_cat_last == "Good" ~ "Improvement",
      Exercise_cat == "Good" & Exercise_cat_last %in% c("Low", "Moderate") ~
        "Worsening",
      Exercise_cat == "Moderate" & Exercise_cat_last == "Low" ~ "Worsening",
      TRUE ~ "No change"
    ),
    ...
    Health_change_first_to_second = case_when(
      ...
      Health_cat == "Low" & Health_cat_second %in% c("Moderate", "Good") ~
        "Improvement",
      Health_cat == "Moderate" & Health_cat_second == "Good" ~ "Improvement",
      Health_cat == "Good" & Health_cat_second %in% c("Low", "Moderate") ~
        "Worsening",
      Health_cat == "Moderate" & Health_cat_second == "Low" ~ "Worsening",
      TRUE ~ "No change"
    ),
    ...
    Health_change_first_to_last = case_when(
      ...
      Health_cat == "Low" & Health_cat_last %in% c("Moderate", "Good") ~
        "Improvement",
      Health_cat == "Moderate" & Health_cat_last == "Good" ~ "Improvement",
      Health_cat == "Good" & Health_cat_last %in% c("Low", "Moderate") ~
        "Worsening",
      Health_cat == "Moderate" & Health_cat_last == "Low" ~ "Worsening",
      TRUE ~ "No change"
    ),
    ...
    BMI_change_first_to_second = case_when(
      ...
      BMI_second - BMI <= -1 ~ "Improvement", # BMI decrease ≥1 unit
      BMI_second - BMI >= 1 ~ "Worsening", # BMI increase ≥1 unit
      TRUE ~ "No change"
    ),

    BMI_change_first_to_last = case_when(
      is.na(BMI) | is.na(BMI_last) ~ NA_character_,
      BMI_last - BMI <= -1 ~ "Improvement",
      BMI_last - BMI >= 1 ~ "Worsening",
      TRUE ~ "No change"
    ),
    ...
    VO2_change_first_to_second = case_when(
      ...
      Astrand_rel_VO2_second - Astrand_rel_VO2 >= 3 ~ "Improvement", # VO2 increase ≥3
      Astrand_rel_VO2_second - Astrand_rel_VO2 <= -3 ~ "Worsening", # VO2 decrease ≥3
      TRUE ~ "No change"
    ),

    VO2_change_first_to_last = case_when(
      is.na(Astrand_rel_VO2) | is.na(Astrand_rel_VO2_last) ~ NA_character_,
      Astrand_rel_VO2_last - Astrand_rel_VO2 >= 3 ~ "Improvement",
      Astrand_rel_VO2_last - Astrand_rel_VO2 <= -3 ~ "Worsening",
      TRUE ~ "No change"
    ),
  ...
  mutate(
    Exercise_change_first_to_second = factor(
      ...
      levels = c("Worsening", "No change", "Improvement")
    ),
...
df_final_analysis <- df_change |>
  # First ensure 1:4 matching if desired
  group_by(id_cluster) |>
  filter(
    n() == 5,
    sum(treated == "HPA") == 1,
  ...
  # Assign second test dates to controls
  mutate(
  ...
  mutate(
    ...
    cvd_event_between_tests = case_when(
      ...
      cvd_art_after_HPA_indatum >= first_test_date &
    ...
    # Time variables
    year_test1 = year(first_test_date),
