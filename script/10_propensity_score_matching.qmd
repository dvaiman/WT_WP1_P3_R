...
Iterate over years to use the matchIt function by year
...
# Ensure 'treated' is binary
# Adjust the conversion based on your data's current encoding
# Example: Convert logical to numeric
...
# Initialize an empty list to store results if needed
# all_matches <- list()
...
for (year in years) {
  ...
  # Subset data for the current year using data.table for efficiency
  subset_data <- combined_data[Year == year]
  ...
  if (!is.null(nearest_neighbor_match)) {
    ...
    # Optionally store in a list (commented out to save memory)
    # all_matches[[as.character(year)]] <- nearest_neighbor_match
    
    # Log the successful processing
    message_text <- paste("Year", year, "processed successfully.")
    writeLines(message_text, con = log_con)
    message(message_text)
  }
  
  # Clear memory
  rm(nearest_neighbor_match, subset_data)
  gc()
...
# files are here, names are matchit_results_1995.rds and so on
rds_dir <- here()

rds_files <- list.files(
  path = rds_dir,
  pattern = "^matchit_results_\\d{4}\\.rds$",  # Regex to match filenames like matchit_results_1995.rds
  full.names = TRUE                           # Return full file paths
)

# Initialize an empty list to store balance statistics
balance_list <- list()
...
for (file_path in rds_files) {
  ...
  # Read the MatchIt object from the .rds file
  matchit_obj <- readRDS(file_path)
  
  # Compute balance statistics using bal.tab from the cobalt package
  balance <- bal.tab(matchit_obj)$Balance
  
  # Add Year information to the balance data
  balance$Year <- year
  
  # Append the balance data to the list with the year as the key
  balance_list[[as.character(year)]] <- balance
}

# Combine all balance statistics into one data frame
combined_balance <- bind_rows(balance_list, .id = "Year")

# View the combined balance statistics
combined_balance %>%  filter(Type=="Distance") %>% summarise(round(mean(Diff.Adj), 10))

combined_balance %>%  rownames_to_column(var = "combined_balance")  %>% filter(Type != "Distance", Diff.Adj > 0.05 | Diff.Adj < -0.05) 

combined_balance %>%  rownames_to_column(var = "combined_balance")  %>% filter(Diff.Adj != 0) 

# Save the combined balance statistics
write.csv(combined_balance, here::here("combined_balance_stats.csv"), row.names = FALSE)
```
...
library(cobalt)     # For balance statistics (if needed)
...
combined_matched_data <- rds_files %>%
  set_names() %>%                        # Optionally preserve file names as names
...
print(head(combined_matched_data))

# Example: Check unique years
print(unique(combined_matched_data$Year))
