...
pacman::p_load(
  ...
  here,
  dtplyr, # For lazy_dt and data.table backend
  data.table, # For efficient data handling with fread and rbindlist
  tictoc, # For timing operations
  progress, # For progress bars
  cobalt, # For covariate balance checking
  gtsummary
)
```
...
Propensity score matching from the total population per year vs the HPI population. 3 to 1.

The propensity score matching will be on variables Sex, birthyear, (Year), income and Education.
...
pop_dt <- fread(
  ...
  select = c("LopNr", "Kon", "Year", "FodelseAr", "edu", "LoneInk"),
  na.strings = c("", "NA"),
  showProgress = TRUE
)
...
pop_df <- lazy_dt(pop_dt) %>%
  ...
  select(LopNr, Sex, Year, FodelseAr, SUN2000Niva_Old, LoneInk) %>%
  filter(Year >= 1995 & Year < 2022) %>%
  as.data.table()
...
combined_data[,
  (categorical_cols) := lapply(.SD, as.factor),
...
for (year in years) {
  ...
  if (length(unique_treated) != 2 || !all(unique_treated %in% c(0, 1))) {
    ...
    message_text <- paste(
      ...
      year,
      ": The treatment must be a binary variable. Found values:",
      paste(unique_treated, collapse = ", ")
    )
  ...
  nearest_neighbor_match <- tryCatch(
    {
      matchit(
        treated ~ Sex + FodelseAr + SUN2000Niva_Old + log(LoneInk),
        data = subset_data,
        method = "nearest",
        distance = "glm",
        link = "logit",
  ...
  if (!is.null(nearest_neighbor_match)) {
    ...
    saveRDS(
      ...
      here(
        ...
        "data",
        "matchit_objects",
        paste0("matchit_results_", year, ".rds")
      )
    )
    ...
    message_text <- paste(
      "Year",
      year,
      "processed successfully. Matched IDs added."
    )
    writeLines(message_text, con = log_con)
    message(message_text)
  }
...
combined_matched_data |>
  select(LoneInk, id_cluster) |>
  group_by(LoneInk) |>
  summarize(mean(log(LoneInk)), n()) |>
  ungroup() |>
  mutate(
    min(log(LoneInk)),
    max(log(LoneInk)),
...
# 5) visual check of balance

balance_long |>
 filter(Type != "Distance") |>
 ggplot(aes(x = year, y = Diff.Adj)) +
 geom_hline(yintercept = c(-0.1, 0.1), linetype = "dashed", colour = "red") +
 geom_hline(yintercept = 0) +
 geom_point(alpha = 0.5) +
 facet_wrap(~covariate, scales = "free_y") +
 labs(title = "Balance after matching (should be near zero)")
```
...
rds_directory <- here("..", "data", "matchit_objects")
...
# med inkomst
reg_ut <- combined_matched_data |> select(LopNr, Year, treated, id_cluster)
write_csv(reg_ut, here::here("..", "data", "registeruttagdata_2026-01-22.csv"))
reg_ut_lopnr <- reg_ut |> select(LopNr) |> distinct(LopNr)
write_csv(
  reg_ut_lopnr,
  here::here("..", "data", "registeruttagdata_lopnr_2026-01-10.csv")
)
```

# check if there are different individuals in these datasts
```{r}
# read both datasets
reg_ut_old <- read_csv(here::here(
  "..",
  "data",
  "registeruttagdata_2025-01-16.csv"
))
reg_ut_new <- read_csv(here::here(
  "..",
  "data",
  "registeruttagdata_2026-01-22.csv"
))

write_csv(
  reg_ut_new |> select(LopNr) |> distinct(LopNr),
  here::here(
    "..",
    "data",
    "registeruttagdata_for_matching_2026-02-06.csv"
  )
)

# check how many unique individuals in each
n_old <- n_distinct(reg_ut_old$LopNr)
n_new <- n_distinct(reg_ut_new$LopNr)

cat("Unique individuals in old dataset:", n_old, "\n")
cat("Unique individuals in new dataset:", n_new, "\n")

# check how many in new are NOT in old
not_in_old <- reg_ut_new |>
  filter(!LopNr %in% reg_ut_old$LopNr) |>
  distinct(LopNr)

cat("Individuals in new but NOT in old:", nrow(not_in_old), "\n")

# check how many in old are NOT in new
not_in_new <- reg_ut_old |>
  filter(!LopNr %in% reg_ut_new$LopNr) |>
  distinct(LopNr)

cat("Individuals in old but NOT in new:", nrow(not_in_new), "\n")

# overlap
overlap <- n_distinct(reg_ut_new$LopNr[reg_ut_new$LopNr %in% reg_ut_old$LopNr])
cat("Overlapping individuals:", overlap, "\n")
```
...
```{r}
# exclude the distance row and focus on actual covariates
balance_stats <- balance_long |>
  filter(Type != "Distance")

# before matching statistics
before_stats <- balance_stats |>
  summarise(
    median_smd_before = median(abs(Diff.Un)),
    min_smd_before = min(abs(Diff.Un)),
    max_smd_before = max(abs(Diff.Un)),
    mean_smd_before = mean(abs(Diff.Un))
  )

# after matching statistics
after_stats <- balance_stats |>
  summarise(
    median_smd_after = median(abs(Diff.Adj)),
    min_smd_after = min(abs(Diff.Adj)),
    max_smd_after = max(abs(Diff.Adj)),
    mean_smd_after = mean(abs(Diff.Adj))
  )

# check how many covariates exceed thresholds
balance_stats |>
  summarise(
    n_above_0.1_before = sum(abs(Diff.Un) > 0.1),
    n_above_0.1_after = sum(abs(Diff.Adj) > 0.1),
    n_above_0.25_before = sum(abs(Diff.Un) > 0.25),
    n_total = n()
  )
...
edu_bal |>
  ...
  facet_wrap(~covariate, scales = "free_y") +
  labs(
    y = "standardized mean difference after matching",
    x = NULL,
    title = "balance on education covariates by year"
  )

# create summary table of balance statistics
balance_summary <- balance_long |>
  filter(Type != "Distance") |>
  mutate(
    covariate_clean = case_when(
      covariate == "log(LoneInk)" ~ "Wage income (log)",
      covariate == "FodelseAr" ~ "Birth year",
      covariate == "Sex_Women" ~ "Sex (women)",
      str_detect(covariate, "SUN2000Niva_Old") ~ "Education level",
      TRUE ~ covariate
    )
  ) |>
  group_by(covariate_clean) |>
  summarise(
    smd_before_mean = mean(abs(Diff.Un)),
    smd_before_min = min(abs(Diff.Un)),
    smd_before_max = max(abs(Diff.Un)),
    smd_after_mean = mean(abs(Diff.Adj)),
    smd_after_min = min(abs(Diff.Adj)),
    smd_after_max = max(abs(Diff.Adj)),
    .groups = "drop"
  ) |>
  mutate(
    before = sprintf(
  ...
  select(covariate_clean, before, after)
...
balance_long |>
  ...
  mutate(
    covariate_clean = case_when(
      ...
      covariate == "FodelseAr" ~ "Birth year",
      covariate == "Sex_Women" ~ "Sex (women)",
      str_detect(covariate, "SUN2000Niva_Old_\\*") ~ "Education (missing)",
      str_detect(covariate, "SUN2000Niva_Old_1") ~ "Education level 1",
      str_detect(covariate, "SUN2000Niva_Old_2") ~ "Education level 2",
      str_detect(covariate, "SUN2000Niva_Old_3") ~ "Education level 3",
      str_detect(covariate, "SUN2000Niva_Old_4") ~ "Education level 4",
      str_detect(covariate, "SUN2000Niva_Old_5") ~ "Education level 5",
      str_detect(covariate, "SUN2000Niva_Old_6") ~ "Education level 6",
      str_detect(covariate, "SUN2000Niva_Old_7") ~ "Education level 7",
      TRUE ~ covariate
    )
  ) |>
  select(covariate_clean, Diff.Un, Diff.Adj, year) |>
  pivot_longer(
  ...
  mutate(
    timing = if_else(timing == "Diff.Un", "Before matching", "After matching"),
    timing = factor(timing, levels = c("Before matching", "After matching"))
  ) |>
  group_by(covariate_clean, timing) |>
  summarise(
  ...
  ggplot(aes(
    x = mean_smd,
    y = reorder(covariate_clean, mean_smd),
    colour = timing
  )) +
  geom_point(size = 3) +
  ...
  scale_colour_manual(
    values = c("Before matching" = "#E69F00", "After matching" = "#0072B2")
  ) +
  labs(
    x = "Absolute standardized mean difference",
    y = NULL,
    colour = NULL,
    caption = "Dashed line indicates SMD = 0.1 threshold"
  ) +
  theme_minimal() +
  theme(
