...
data_cvd <- read_csv(here::here("..", "data", "df_paper_3_CVD_2025-03-05.csv")) |> 
  left_join(read_csv(here::here("..", "data", "registeruttagdata_2025-01-16.csv")) |> select(LopNr, id_cluster)) |> 
  left_join(read_csv(here::here("..", "data", "df_n_tests_CVD_2025-03-05.csv")) |> 
              select(LopNr, n_tests_total, n_tests_within_5, n_tests_within_10, composite_score_within_5, composite_score_within_10, composite_score_total,
                     ExerciseAnswer, TobaccoSmoking, Health, StressOverall, Astrand_rel_VO2, BMI))
...
data_cvd <-
  ...
  mutate(
    ...
    birth_cohort = factor(birth_cohort, levels = c(
      "≤39",
      "1940–49",
      "1950–59",
      "1960–69",
   ...
   # age_exit = if_else(age_exit - age_enter == 0, 1 + age_exit, age_exit),
    ...
    n_tests_within_5_fac = factor(n_tests_within_5_fac, levels = c("Control", "HPA, 1 test", "HPA, 2 tests", "HPA, ≥3 tests")),
   n_tests_within_10_fac = case_when(
    ...
    n_tests_within_10_fac = factor(n_tests_within_10_fac, levels = c("Control", "HPA, 1 test", "HPA, 2 tests", "HPA, ≥3 tests")),
    ...
    across(c(n_tests_total, n_tests_within_5, composite_score_within_5, composite_score_total), 
           ~ if_else(is.na(.), 0, .)),
    ...
    EducationLevel = factor(EducationLevel, levels = c("Primary", "Secondary", "Tertiary")),
    ...
    ssyk_WBHL_group = factor(ssyk_WBHL_group, levels = c(
    ...
    KommunSize = factor(KommunSize, 
                        levels = c("Metropolitan municipalities", 
                                   "Dense municipalities",
                                   "Rural municipalities")),
   # Place_of_origin = if_else(is.na(Place_of_origin), "Unknown", Place_of_origin),
    Place_of_origin = factor(Place_of_origin, 
                             levels = c("Outside Europe", 
                                        "Europe", 
                                        "Sweden")),
    ...
    Income_Sources_Category = factor(Income_Sources_Category, 
                                     levels = c(">3", "1", "2 to 3")),
    Turnover_Rate_Category = factor(Turnover_Rate_Category, 
                                    levels = c("<10%", "≥20%", "10% to <20%")),
    IncomeLevel_CSFVI = factor(IncomeLevel_CSFVI, 
                               levels = c("<60%", "60–79%", "80–119%", "120–199%", "≥200%")),
    Income_Sources_Category	 = factor(Income_Sources_Category	, 
                                      levels = c("1", "2", ">3")),
    Turnover_Rate_Category	 = factor(Turnover_Rate_Category, 
                                     levels = c("<10%", "10% to <20%", "≥20%")),
    Operating_Profit_Margin_Category	 = factor(Operating_Profit_Margin_Category, 
                                               levels = c("<-5%", "-5% to <0%", "0% to 5%", ">5%")),
    Number_of_employees_Category	 = factor(Number_of_employees_Category, 
                                           levels = c("1 to 9", "10 to 49", "50 to 249", "≥250")),
    cvd_art_before_HPA_flag = if_else(is.na(cvd_art_before_HPA_flag), 0, cvd_art_before_HPA_flag),
  ...
  group_by(id_cluster) %>% 
  mutate(
    BMI_cat      = factor(replace_na(BMI_cat_tmp,      first(na.omit(BMI_cat_tmp))),
                          levels = c("Underweight", "Normal weight", "Overweight", "Obesity")),
    vo2_cat      = factor(replace_na(vo2_cat_tmp,      first(na.omit(vo2_cat_tmp))),
                          levels = c("Very low","Low","Moderate","High","Very high")),
    Health_cat   = factor(replace_na(Health_cat_tmp,   first(na.omit(Health_cat_tmp))),
                          levels = c("Low","Moderate","Good")),
    Exercise_cat = factor(replace_na(Exercise_cat_tmp, first(na.omit(Exercise_cat_tmp))),
                          levels = c("Low","Moderate","Good")),
    n_tests_within_5_fac_vs_ctrl = factor(replace_na(n_tests_within_5_fac_vs_ctrl, first(na.omit(n_tests_within_5_fac_vs_ctrl))),
